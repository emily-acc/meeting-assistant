...
const enhancedTextParser = {
  ...
  extractTime: (text) => {
    let m = text.match(/上午\s*(\d{1,2}):(\d{2})/);
    if (m) {
      let h = parseInt(m[1]);
      if (h === 12) h = 0;
      return `${String(h).padStart(2,'0')}:${m[2]}`;
    }
    m = text.match(/下午\s*(\d{1,2}):(\d{2})/);
    if (m) {
      let h = parseInt(m[1]);
      if (h !== 12) h += 12;
      return `${String(h).padStart(2,'0')}:${m[2]}`;
  ...
  extractMeetingNumber: (text) => {
    let m = text.match(/(?:号[码碼]|[識識别]別[碼码])[：:\s]+([0-9\s]+)/i);
  ...
  extractLocation: (text) => {
    const m = text.match(/(?:引擎|类型|會議類型)[：:]\s*([^\n]+)/i);
    if (m) return m[1].trim();
...
export default function Home() {
  ...
  const handlePasteMeeting = async (text) => {
    ...
    // 否則用 AI
    try {
      const ai = await identifyWithAI(text, GEMINI_API_KEY);
      if (ai) {
        const meeting = {
          title: ai['會議標題'] || local.title,
          startDate: ai['開始日期'] || local.startDate,
          startTime: ai['開始時間'] || local.startTime,
          endTime: ai['結束時間'] || local.endTime,
          location: ai['會議地點'] || local.location,
          chairman: ai['主持人名稱'] || '',
          password: ai['會議密碼'] || local.password,
          link: ai['會議連結'] || local.link,
          meetingNumber: ai['會議識別碼'] || local.meetingNumber,
          attendees: ai['參加對象'] || local.attendees,
          originalText: text
        };
